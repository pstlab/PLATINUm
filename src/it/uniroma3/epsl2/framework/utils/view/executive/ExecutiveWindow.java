package it.uniroma3.epsl2.framework.utils.view.executive;
...
public class ExecutiveWindow extends JFrame {
	...
	private IntervalCategoryDataset setupDataSet(long units) 
	{
		...
		for (ExecutionNode node : this.dataset) 
		{
			...
			if (!index.get(node.getComponent()).containsKey(node.getPredicateSignature())) {
				index.get(node.getComponent()).put(node.getPredicateSignature(), new ArrayList<ExecutionNode>());
			}
			
			// add node
			index.get(node.getComponent()).get(node.getPredicateSignature()).add(node);
		}
		...
		for (String component : index.keySet()) 
		{
			...
			for (String predicate : index.get(component).keySet()) 
			{
				...
				if (nodes.size() > 1) 
				{
					...
					{
						// create a sub-task
						Task subTask = new Task(node.getPredicateSignature(),
