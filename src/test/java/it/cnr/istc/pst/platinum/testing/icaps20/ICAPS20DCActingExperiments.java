...
//	};
	...
	{
		int horizon = 100; int uncertainty = 10; int window = 1; int instrument = 1; int g = 1;
		...
		try
		{
			...
			try (PrintWriter writer = new PrintWriter(new BufferedWriter(new FileWriter(dataFile)))) {
				...
				writer.println(
						...
						+ "Exists valid strategy;"
						+ "Strategy Management Time (msecs);"
						+ "Is plan out of bounds;"
						+ "Did strategy end"
						);
			}
													...
													for (Goal goal : goals) 
													{
														...
														try (PrintWriter writer = new PrintWriter(new BufferedWriter(new FileWriter(dataFile, true)))) {
															...
															writer.println(
																	...
																	+ goal.getStatus() + ";"
																	+ goal.getUppaalTime() + ";"
																	+ goal.isExistsStrategy() + ";"
																	+ goal.getManagementStrategyTime() + ";"
																	+ goal.isOutOfBounds() + ";"
																	+ goal.isStrategyFinished()
																	);
														}
													}
												}
