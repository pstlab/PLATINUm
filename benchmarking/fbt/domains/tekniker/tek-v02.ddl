DOMAIN FourByThree_TeknikerDomain
{
		...
		At(position, level), _MotionTask(position, position, level))
	{
		...
		VALUE _MotionTask(?from, ?to, ?level) [1, +INF]
		MEETS {
			At(?position, ?l);
			?to = ?position;
			?l = ?level;
	...
	SYNCHRONIZE RobotController.controller
	{
		...
		{
			cd0 <?> RobotArm.motion.At(?initialPosition, ?level0);
			cd1 RobotArm.motion.At(?intermediatePosition, ?level1);
			cd2 RobotArm.motion.At(?finalPosition, ?level2);
			cd3 T1.tool.Operating();
			
			MET-BY cd0;
			CONTAINS [0, +INF] [0, +INF] cd1;
			CONTAINS [0, +INF] [0, +INF] cd3;
			cd1 BEFORE [0, +INF] cd3;
			ENDS-DURING [0, +INF] [0, +INF] cd2;
			
			?intermediatePosition = P4;
			?finalPosition = P3;
			?level1 = green;
			?level2 = green;
		}
