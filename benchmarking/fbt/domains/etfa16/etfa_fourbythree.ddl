...
{
	TEMPORAL_MODULE temporal_module = [0, 80], 200;
	...
	COMP_TYPE SingletonStateVariable AssemblyProcessType (SetTheWorkPiece(workpiece_id), 
		RemoveTopCover(workpiece_id), TurnPiece(workpiece_id), RemoveBottomCover(workpiece_id),
		RemoveTheInsert(workpiece_id),
		RebuildThePiece(workpiece_id), CoverPlacement(workpiece_id))
	{
		...
		MEETS {
			SetTheWorkPiece(?otherPiece);
		}
	}
	
	COMP_TYPE SingletonStateVariable HumanOperatorFunctionType(Idle(), 
		Place(workpiece_id), 
		Screw(workpiece_id), Unscrew(workpiece_id), 
		Rotate(workpiece_id), 
		RemovePart(workpiece_id), RebuildPart(workpiece_id)) 
	{
		VALUE Idle() [1, +INF]
		MEETS {
			Place(?piece0);
	...
	SYNCHRONIZE AssemblyProcess.tasks
	{
		...
		{
			cd0 <!> Human.operator.RebuildPart(?piece0);
