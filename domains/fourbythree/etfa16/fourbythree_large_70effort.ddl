DOMAIN FourByThree_ETFA16_l70e
{
	TEMPORAL_MODULE temporal_module = [0, 110], 200;
	...
	COMP_TYPE SingletonStateVariable AssemblyProcessType (None(),
		...
		RemoveTopCover1(workpiece_id),
		RemoveTopCover2(workpiece_id),
		TurnPiece(workpiece_id), 
		RemoveBottomCover1(workpiece_id),
		RemoveBottomCover2(workpiece_id),
		RemoveInsert(workpiece_id),
		RebuildThePiece(workpiece_id), 
		CoverPlacement1(workpiece_id),
		CoverPlacement2(workpiece_id))
	{
		VALUE None() [1, +INF]
		MEETS {
			SetTheWorkPiece(?p);
		...
		VALUE RemoveTopCover1(?piece) [1, +INF]
		MEETS {
			RemoveTopCover2(?p);
			?p = ?piece;
		}
		
		VALUE RemoveTopCover2(?piece) [1, +INF]
		MEETS {
			TurnPiece(?p);
			?p = ?piece;
		}
		...
		MEETS {
			RemoveBottomCover1(?p);
			?p = ?piece;
		} 
		
		VALUE RemoveBottomCover1(?piece) [1, +INF]
		MEETS {
			RemoveBottomCover2(?p);
			?p = ?piece;
		}
		
		VALUE RemoveBottomCover2(?piece) [1, +INF]
		MEETS {
			RemoveInsert(?p);
			?p = ?piece;
		}
		...
		VALUE CoverPlacement1(?piece) [1, +INF]
		MEETS {
			CoverPlacement2(?p);
			?p = ?piece;
		}  
		
		VALUE CoverPlacement2(?piece) [1, +INF]
		MEETS {
			None();
		}
	}
	...
	SYNCHRONIZE ALFA.use_case
	{
		VALUE Assembly(?piece) 
		{
			...
			task7 <!> AssemblyProcess.tasks.CoverPlacement2(?piece7);			
	...
	SYNCHRONIZE AssemblyProcess.tasks
	{
		...
		VALUE RemoveTopCover1(?piece)
		{
			cd0 <!> RobotController.functions.Unscrew(?piece0, ?location0);
		...
		VALUE RemoveBottomCover1(?piece)
		{
		...
		VALUE RemoveBottomCover2(?piece)
		{
			cd0 <!> RobotController.functions.Unscrew(?piece0, ?location0);
			cd1 <!> RobotController.functions.Unscrew(?piece1, ?location1);
			cd2 CollaborationType.modality.Simultaneous();
		...
		VALUE CoverPlacement1(?piece) 
		{
			cd0 <!> RobotController.functions.Screw(?piece0, ?location0);
