DOMAIN ICO_v1 {
	...
	TEMPORAL_MODULE temporal_module = [0, 70], 300;
	...
	COMP_TYPE SingletonStateVariable RobotArmControllerType (
		...
		VALUE AssumeManipulationPose() [1, +INF]
		MEETS {
			PlaceObject(?obj1, ?loc1, ?plArea1);
			PickUpObject(?obj2, ?loc2, ?plArea2);
		}
		
		VALUE AssumeDrivingPose() [1, +INF]
		MEETS {
			Idle();
		}
	}
	...
	SYNCHRONIZE Mission.ico {
		...
		VALUE ServeCoffeeTo(?guest) {
			...
			// check guest location
			?guestLocation = manipulationAreaNorthTable1;
			?mugFinalLocation = manipulationAreaWestTable1;
			?mugFinalPlacingArea = placingAreaLeft;
	...
	SYNCHRONIZE RobotController.robot {
		...
		VALUE GetObject(?object, ?location) {
			...
			cd3 <!> LeftArmController.left_arm.AssumeManipulationPose();
	...
	SYNCHRONIZE LeftArmController.left_arm {
		...
		VALUE AssumeDrivingPose() {
			...
			ENDS-DURING [1, +INF] [1, +INF] cd1;
			ENDS-DURING [1, +INF] [1, +INF] cd2;
	...
	SYNCHRONIZE RobotBase.base {
		...
		VALUE _MovingTo(?destination) {
			...
			cd0 LeftArmController.left_arm.AssumeDrivingPose();
